...
const WhatWeProvide = () => {
  ...
  const getColorClasses = (color) => color === 'teal'
    ? { gradient: 'from-[#021E14] to-[#021E14]', light: 'bg-[#021E14]/10', text: 'text-[#021E14]', border: 'border-[#021E14]/20' }
    : { gradient: 'from-[#021E14] to-[#021E14]', light: 'bg-[#021E14]/10', text: 'text-[#021E14]', border: 'border-[#021E14]/20' };

  return (
    <section className="relative py-24 overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-[10%] w-72 h-72 bg-gradient-to-br from-[#021E14]/10 to-transparent rounded-full blur-3xl" />
        <div className="absolute bottom-20 right-[10%] w-80 h-80 bg-gradient-to-br from-[#021E14]/10 to-transparent rounded-full blur-3xl" />
      </div>
      ...
      <div className="container mx-auto px-4 lg:px-16 relative z-10">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <motion.div className="inline-flex items-center gap-3 mb-5 px-5 py-2.5 rounded-full bg-white dark:bg-black/50 border border-[#021E14]/30 shadow-sm" initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
            <div className="w-8 h-8 rounded-lg bg-gradient-to-r from-[#021E14]/20 to-cyan-500/20 flex items-center justify-center">
              <HiOutlineSparkles className="text-[#021E14]" size={14} />
            </div>
        ...
        <motion.div className="flex justify-center mt-14" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.5 }}>
          <Link href="/about" className={`group relative bg-white dark:bg-[#0d0d0d] rounded-2xl px-8 py-4 border border-gray-200 dark:border-white/10 hover:-translate-y-1 hover:shadow-lg flex items-center gap-4 transition-all ${bengaliClass}`}>
            ...
            <div className="w-10 h-10 rounded-xl bg-[#021E14]/10 flex items-center justify-center group-hover:bg-[#021E14]"><LuArrowRight size={18} className="text-[#021E14] group-hover:text-white" /></div>
          </Link>
        </motion.div>
      </div>
    </section>
