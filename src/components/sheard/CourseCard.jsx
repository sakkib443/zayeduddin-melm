      ...
      >
        ...
        <div className="relative w-full md:w-[35%] h-56 md:h-auto shrink-0 overflow-hidden p-3">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/20 backdrop-blur-[1px]">
            <Link href={`/courses/${courseId}`} className="w-14 h-14 bg-white/30 backdrop-blur-md rounded-md flex items-center justify-center text-white border border-white/50 hover:bg-[#300000] hover:border-[#300000] transition-all hover:scale-110">
        ...
        <div className="w-full md:w-[25%] p-6 bg-slate-50/50 dark:bg-white/5 flex flex-col items-center justify-center text-center gap-1 border-l border-slate-100 dark:border-white/5">
          ...
          <div className="flex items-center gap-3 w-full">
            <button
              ...
              disabled={isAdded}
              className={`p-2.5 border rounded-md transition-all shadow-sm ${isAdded ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white dark:bg-white/10 border-slate-200 dark:border-white/10 text-slate-600 dark:text-white hover:text-[#300000] hover:border-[#300000]'}`}
            >
              {isAdded ? <LuCheck size={20} /> : <LuShoppingCart size={20} />}
            </button>
            <Link
              href={`/courses/${courseId}`}
              className="flex-1 py-2.5 bg-white dark:bg-white/10 border border-[#300000] text-[#300000] rounded-md text-sm font-medium hover:bg-[#300000] hover:text-white transition-all shadow-sm flex items-center justify-center gap-2"
            >
              Details
            </Link>
          </div>
    ...
    >
      <div className={`relative h-full bg-white dark:bg-[#0d0d0d] rounded-md border border-slate-100 dark:border-white/10 overflow-hidden hover:shadow-2xl hover:border-[#300000]/20 transition-all duration-500 flex flex-col shadow-lg shadow-black/5`}>
        ...
        <div className="relative h-64 w-full overflow-hidden shrink-0">
          ...
          >
            <Link href={`/courses/${courseId}`} className="w-14 h-14 bg-white/20 backdrop-blur-md rounded-md flex items-center justify-center text-white border-2 border-white/40 hover:bg-[#300000] hover:border-[#300000] transition-all hover:scale-110 shadow-2xl">
              <LuPlay className="ml-1" size={20} fill="currentColor" />
            </Link>
          </motion.div>
        </div>
