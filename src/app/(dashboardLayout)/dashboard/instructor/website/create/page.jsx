    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                router.push('/dashboard/instructor/website');
            } else {
        ...
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/instructor/website" className="p-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl transition-all">
                        <FiArrowLeft size={18} />
                    </Link>
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-2 bg-gradient-to-r from-[#021E14] to-[#01140D] hover:from-[#021E14] hover:to-[#01140D] text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-lg shadow-[#021E14]/25 transition-all disabled:opacity-50"
                >
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    {errors.platform && <p className="text-[#021E14] text-xs mt-1">{errors.platform.message}</p>}
                                </div>
                                <div>
                    ...
                    <div className={cardClass}>
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button type="button" onClick={() => techFields.append('')} className="text-xs font-medium text-[#021E14] hover:text-emerald-700">+ Add</button>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {techFields.fields.map((field, idx) => (
                                    <div key={field.id} className="group relative">
                                        ...
                                        <button type="button" onClick={() => techFields.remove(idx)} className="absolute -top-1 -right-1 bg-[#021E14] text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-all"><FiTrash2 size={10} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-slate-100">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <button type="button" onClick={() => featureFields.append('')} className="text-xs font-medium text-[#021E14] hover:text-emerald-700">+ Add</button>
                            </div>
                            <div className="space-y-2">
                                {featureFields.fields.map((field, idx) => (
                                    <div key={field.id} className="flex gap-2">
                                        ...
                                        <button type="button" onClick={() => featureFields.remove(idx)} className="text-[#021E14] p-2"><FiTrash2 /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <button type="button" onClick={() => imageFields.append('')} className="text-xs font-medium text-[#021E14] hover:text-emerald-700">+ Add</button>
                                </div>
                                <div className="space-y-2">
                                    {imageFields.fields.map((field, idx) => (
                                        <div key={field.id} className="flex gap-2 relative">
                                            ...
                                            <button type="button" onClick={() => imageFields.remove(idx)} className="text-[#021E14] p-2 hover:text-[#021E14]"><FiTrash2 size={16} /></button>
                                        </div>
                                    ))}
                                    {errors.images && <p className="text-[#021E14] text-xs">{errors.images.message || errors.images[0]?.message}</p>}
                                </div>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input type="number" {...register('price')} className="w-full bg-slate-700 border border-slate-600 rounded-xl py-3 pl-10 pr-4 text-white text-lg focus:ring-2 focus:ring-[#021E14] focus:border-transparent" placeholder="0" />
                                </div>
                                {errors.price && <p className="text-[#021E14] text-xs mt-1">{errors.price.message}</p>}
                            </div>
                            <div>
                                <label className="text-sm font-medium text-slate-300 block mb-2">Offer Price (?)</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-3 text-slate-400">?</span>
                                    <input type="number" {...register('offerPrice')} className="w-full bg-slate-700 border border-slate-600 rounded-xl py-3 pl-10 pr-4 text-white text-lg focus:ring-2 focus:ring-[#021E14] focus:border-transparent" placeholder="0" />
                                </div>
                            </div>
