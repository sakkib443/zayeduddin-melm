    ...
    <div className={`flex items-start gap-3 p-3 transition-all ${isNew ? (isDark ? 'bg-blue-500/10' : 'bg-blue-50') : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {isNew && <span className="w-1.5 h-1.5 bg-[#021E14] rounded-full" />}
        </div>
...
export default function AdminDashboard() {
  ...
  const productStats = [
    { title: 'Courses', value: dashboardData.totalCourses, icon: FiBook, color: 'bg-[#021E14]', href: '/dashboard/admin/course' },
    { title: 'Software', value: dashboardData.totalSoftware, icon: FiCode, color: 'bg-cyan-600', href: '/dashboard/admin/software' },
    { title: 'Websites', value: dashboardData.totalWebsites, icon: FiGlobe, color: 'bg-[#021E14]', href: '/dashboard/admin/website' },
    { title: 'Categories', value: dashboardData.categories, icon: FiLayers, color: 'bg-[#D4AF37]', href: '/dashboard/admin/category' },
  ];

  const quickActions = [
    { title: 'Add Course', href: '/dashboard/admin/course/create', icon: FiBook, color: 'bg-[#D4AF37]' },
    { title: 'Add Website', href: '/dashboard/admin/website/create', icon: FiGlobe, color: 'bg-[#021E14]' },
    { title: 'Add Software', href: '/dashboard/admin/software/create', icon: FiCode, color: 'bg-cyan-600' },
    { title: 'Add Category', href: '/dashboard/admin/category/create', icon: FiLayers, color: 'bg-[#021E14]' },
  ];
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-md border p-4 ${isDark
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-[#021E14] hover:bg-[#01140D] text-white rounded-md text-sm font-normal transition-all">
            <FiDownload size={14} />
            Export
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className={`p-5 rounded-md border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-emerald-100 text-[#021E14] rounded-md flex items-center justify-center">
                <FiAward size={18} />
              </div>
              <div>
            ...
            {loading ? (
              ...
              topCourses.slice(0, 4).map((course, idx) => (
                <div key={course._id || idx} className={`flex items-center gap-3 p-2 rounded-md transition-colors ${isDark ? 'hover:bg-slate-700/50' : 'hover:bg-gray-50'}`}>
                  <div className={`w-8 h-8 rounded-md flex items-center justify-center text-white font-medium text-sm ${['bg-[#021E14]', 'bg-[#D4AF37]', 'bg-[#021E14]', 'bg-[#021E14]'][idx % 4]}`}>
                    {idx + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <span className="text-sm font-medium text-[#021E14]">৳{course.price || 0}</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className={`p-5 rounded-md border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          ...
          <div className="space-y-2">
            ...
            <div className={`flex justify-between items-center p-3 rounded-md ${isDark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
              ...
              <span className="text-base font-semibold text-[#021E14]">+{dashboardData.newUsersThisMonth}</span>
            </div>
            <div className={`flex justify-between items-center p-3 rounded-md ${isDark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
              <span className={`text-sm font-normal ${isDark ? 'text-slate-300' : 'text-gray-600'}`}>Active Enrollments</span>
              <span className="text-base font-semibold text-[#021E14]">{dashboardData.activeEnrollments}</span>
            </div>
          </div>
        </div>
      </div>
                ...
                {recentOrders.length === 0 ? (
                  ...
                  recentOrders.map((order) => (
                    <tr key={order.id} className={`transition-colors ${isDark ? 'hover:bg-slate-700/30' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-md bg-[#021E14] flex items-center justify-center text-white text-xs font-medium">
                      ...
                      <td className="p-3 text-sm font-medium text-[#021E14]">৳{order.amount?.toLocaleString()}</td>
                      <td className="p-3">
