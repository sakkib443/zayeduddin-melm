...
export default function TermsPage() {
    ...
    const fetchPage = async () => {
        try {
            const res = await fetch(`${API_URL}/legal-pages/terms`);
                ...
                <div className="container mx-auto px-4 lg:px-16 relative z-10 text-center">
                    ...
                    <div className="flex items-center justify-center gap-4 mb-3">
                        <div className="w-12 h-12 bg-emerald-100 dark:bg-emerald-900/30 rounded-xl flex items-center justify-center">
                            <FiFileText className="text-emerald-600 dark:text-emerald-400" size={24} />
                        </div>
                        <h1 className="text-3xl lg:text-4xl font-bold text-[#021E14] dark:text-white">{title || 'Terms & Conditions'}</h1>
                    </div>
                    {page?.updatedAt && (
